...
<div class="flex w-full flex-col gap-5">
	...
	<div class="flex h-full flex-col gap-4 pt-4 max-sm:pt-0">
		{#if envPublic.PUBLIC_APP_DATA_SHARING === "1"}
			...
			<label class="flex items-center">
				...
				<div class="inline cursor-pointer select-none items-center gap-2 pl-2">
					Chia sẻ đoạn hội thoại với các tác giả của mô hình
				</div>
			</label>

			<p class="text-sm text-gray-500">
				Chia sẻ dữ liệu của bạn sẽ giúp chúng tôi phát triển mô hình tốt hơn nữa.
			</p>
		{/if}
		<!-- svelte-ignore a11y-label-has-associated-control -->
		<label class="mt-6 flex items-center">
			...
			<div class="inline cursor-pointer select-none items-center gap-2 pl-2">
				Ẩn biểu tượng cảm xúc trong các chủ đề trò chuyện
			</div>
		</label>
		...
		<label class="mt-6 flex items-center">
			...
			<div class="inline cursor-pointer select-none items-center gap-2 pl-2">
				Vô hiệu hóa tính năng tạo sinh từ ngữ trực tiếp
			</div>
		</label>
		...
		<div class="mt-12 flex flex-col gap-3">
			...
			<button
				...
				type="submit"
				class="flex items-center underline decoration-gray-300 underline-offset-2 hover:decoration-gray-700"
				><CarbonTrashCan class="mr-2 inline text-sm text-red-500" />Xóa tất cả cuộc trò chuyện</button
			>
		</div>
	</div>
